...
# TOKENS (If the DESTINATION is PROD , provide project level token)

SRC_TOKEN="BBDC-NDc3NjcwNzcwMjE2OpGvcSP8mwRnodmm5pLZl5eD+Mur"            #(https://git-staging.gradientai.systems)
DEST_TOKEN="BBDC-OTU1NTQ5NzE5MDAwOjlCt1HbfdjRjFHbnxiNsjLelwv+"      #(https://git.gradientai.systems)
...
"https://git-staging.gradientai.systems/rest/api/1.0/projects/$src_project_key/repos?limit=1000" \
| jq -r '.values[].slug')
...
do
    ...
    source_repo_url="https://git-staging.gradientai.systems/scm/$src_project_key/$repo_name.git"
